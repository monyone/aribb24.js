...
export default (caption: CaptionData) => {
  ...
  switch (caption.tag) {
    case 'CaptionManagement': {
      ...
      const data_group_size = data_unit_loop.byteLength + 3 + 1 /* TMD */ + 1 + languages.byteLength;
      const buffer = new ArrayBuffer(3 + (2 + data_group_size) + 2 /* CRC16*/);
      const array = new Uint8Array(buffer);
      const view = new DataView(buffer);
      view.setUint8(0, (caption.group << 7) | (0 << 2));
      view.setUint16(3, data_group_size, false);
      view.setUint8(3 + 2 + 0, 0 << 6); // TODO: TMD only 0 is supported
      view.setUint8(3 + 2 + 1, caption.languages.length);
      array.set(new Uint8Array(languages), 7);
      view.setUint8(3 + 2 + 2 + languages.byteLength + 0, data_unit_loop.byteLength >> 16);
      view.setUint16(3 + 2 + 2 + languages.byteLength + 1, data_unit_loop.byteLength & 0xFFFF, false);
    ...
    case 'CaptionStatement': {
      const data_group_size = data_unit_loop.byteLength + 3 + 1 /* TMD */;
      const buffer = new ArrayBuffer(3 + (2 + data_group_size) + 2 /* CRC16*/);
      const array = new Uint8Array(buffer);
      const view = new DataView(buffer);
      ...
      view.setUint16(3, data_group_size, false);
      view.setUint8(3 + 2 + 0, 0 << 6); // TODO: TMD only 0 is supported
      view.setUint8(3 + 2 + 1 + 0, data_unit_loop.byteLength >> 16);
      view.setUint16(3 + 2 + 1 + 1, data_unit_loop.byteLength & 0xFFFF, false);
      array.set(new Uint8Array(data_unit_loop), 3 + 2 + 1 + 3);
      const CRC16 = CRC16_CCITT(array, 0, array.byteLength - 2);
      view.setUint16(array.byteLength - 2, CRC16, false);
      return buffer;
    }
