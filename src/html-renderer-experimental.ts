...
export default class HTMLID3Renderer {
  ...
  public pushRawData(pts: number, data: Uint8Array): boolean {
    const provider: HTMLProvider = new HTMLProvider(data, pts);
    const estimate = provider.render(this.rendererOption)
  ...
  private addB24Cue (start_time: number, end_time: number, data: Uint8Array): boolean {
    ...
    if (!HTMLProvider.detect(data, this.rendererOption)) { return false; }
  ...
  private onB24CueChange() {
    ...
    if (this.b24Track.activeCues && this.b24Track.activeCues.length > 0) {
      ...
      if ((lastCue.startTime <= this.media.currentTime && this.media.currentTime <= lastCue.endTime) && !this.isOnSeeking) {
        ...
        const provider: HTMLProvider = new HTMLProvider(lastCue.data, lastCue.startTime);
        let rendered = false
  ...
  private onResize() {
    ...
    const media_width = Number.parseInt(style.width)
    const media_height = Number.parseInt(style.height)

    // TODO;
    const caption_width = this.wrapper.offsetWidth
    const caption_height = this.wrapper.offsetHeight

    const scale_x = media_width / caption_width
    const scale_y = media_height / caption_height

    if (this.rendererOption?.keepAspectRatio) {
      const scale = Math.min(scale_x, scale_y);

      this.wrapper.style.top = `${(media_height - (caption_height * scale)) / 2}px`;
      this.wrapper.style.left = `${(media_width - (caption_width * scale)) / 2}px`;
      this.wrapper.style.transform = `scale(${scale})`
      this.wrapper.style.transformOrigin = '0 0';
    } else {
      this.wrapper.style.top = '0px';
      this.wrapper.style.left = '0px';
      this.wrapper.style.transform = `scale(${scale_x}, ${scale_y})`
      this.wrapper.style.transformOrigin = '0 0';
    }
  ...
  private setupTable(): void {
    ...
    this.subtitleElement.appendChild(this.wrapper)
  ...
  private cleanupTable(): void {
    ...
    if (this.table && this.wrapper) {
      this.wrapper.removeChild(this.table)
    }
    if (this.wrapper && this.subtitleElement) {
      this.subtitleElement.removeChild(this.wrapper)
    }
    this.wrapper = this.table = null
  }
}
