...
const cmdline = ([
  ...
  },
  {
    long: '--ruby',
    short: '-r',
    help: 'Specify ruby (preserve, ignore)',
    action: 'default',
  },
  {
    long: '--help',
...
(async () => {
  ...
  const font = cmd['font'];
  const ruby = cmd['ruby'] === 'ignore' ? SSZ_RUBY_DETECTION.IGNORE : SSZ_RUBY_DETECTION.PRESERVE;
  if (font == null) {
  ...
  let captions: [begin: number, end: number, regions: ARIBB24Region[]][] = [];
  {
    ...
    for await (const independent of read(await readableStream(input))) {
      ...
      if (caption.tag === 'CaptionManagement') {
        ...
        captions.push([independent.pts, elapsed_time ===  0 ? Number.POSITIVE_INFINITY : independent.pts + elapsed_time, regioner(tokens, info, ruby)]);
  ...
  if (captions.length >= 1 && !Number.isFinite(!captions[captions.length - 1][1])) {
